...
@section('content')
    ...
    <form action="{{ route('fuel-entries.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" class="form-control" id="total_cost" name="total_cost" value="{{ old('total_cost') }}" required>
        </div>

        <div class="mb-3">
            <label for="fuel_amount_liters_display" class="form-label">Fuel Amount (Liters) (Auto-calculated)</label>
            <input type="text" class="form-control" id="fuel_amount_liters_display" disabled>
        </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const totalCostInput = document.getElementById('total_cost'); // Changed
            const fuelAmountLitersDisplay = document.getElementById('fuel_amount_liters_display'); // Changed
            ...
            // Function to calculate fuel amount liters
            function calculateFuelAmountLiters() {
                const price = parseFloat(fuelPriceInput.value);
                const cost = parseFloat(totalCostInput.value); // Changed

                if (!isNaN(price) && !isNaN(cost) && price > 0) {
                    const fuelAmount = (cost / price).toFixed(2); // Calculate and round to 2 decimal places
                    fuelAmountLitersDisplay.value = fuelAmount;
                } else {
                    fuelAmountLitersDisplay.value = '';
                }
            }

            vehicleSelect.addEventListener('change', getFuelType);
            fuelPriceInput.addEventListener('input', calculateFuelAmountLiters); // Changed
            totalCostInput.addEventListener('input', calculateFuelAmountLiters);   // Changed

            // Initial calls if values are pre-filled (e.g., on form validation error)
            getFuelType();
            calculateFuelAmountLiters();
        });
    </script>
@endsection